...
export interface ParserContext {
    ...
    resolveInput(node: BotNode, key: string): AST.Expression;

    // Get a control/data value directly
    getNodeValue(node: BotNode, key: string): any;

    // Traverse a flow output (e.g. 'exec', 'true', 'loopBody') returning a Block
    traverseBlock(node: BotNode, outputKey: string): AST.BlockStatement;
